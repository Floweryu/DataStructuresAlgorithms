...
{
	(*L).elem=(ElemType*)malloc(maxnum*sizeof(ElemType));	//申请存储空间 
	if((*L).elem==NULL)	//判断申请是否成功 
	return(OVERFLOW);
	else
	{
		(*L).length=0;	//初始长度为0 
		(*L).listsize=maxnum;	//定义一个最大值	 
		return OK;
	}
}
//给一个空线性表赋值 
...
Status CreateList(SqList *L,int n)
{
	for(int i=0;i<n;i++)
	{
		scanf("%d",&(*L).elem[i]);	//向表中写入数据 
		(*L).length++;	//写入一个，实际长度加一 
		if((*L).length>=(*L).listsize)	//当实际长度大于最大长度时 
		{
			ElemType *newbase=(ElemType*)realloc((*L).elem,((*L).listsize+LISTINCREMENT)*sizeof(ElemType));	//在原来基础上接着申请空间 
			if(newbase==NULL)	//判断是否申请成功 
			exit(OVERFLOW);
			(*L).elem=newbase;	//线性表的基址发生改变 
...
Status DeleteReList(SqList *L)
{
	...
	p=(*L).elem;
	q=p+1;
	for(i=1;i<=(*L).length;i++)		//类似于冒泡排序法 
	{
		if(*p==*q)	//找到相邻的相同的元素 
		{
			p=q;
			q=q+1;	//移动指针 
			iDeleteList(L,i,&E);	//调用函数删除 
		}
		else
		{
			p=q;
...
Status eInsertList(SqList *L,ElemType e,ElemType E)
{
	ElemType *q,*p,*newbase;
	int i;
	i=LocateElemList(L,e);	//找到值为e的元素的位置 
	iInsertList(L,i,E);		//在该位置前插入元素E 
 } 
...
int main()
{
		...
		printf("      *****功能：删除有序线性表中重复的元素。                   please enter  j   **\n");
